...
function ShopPageContent() {
  ...
  useEffect(() => {
    ...
    // Filter by category (slug: velas, roupa, decorative-elements)
    if (selectedCategory) {
      filtered = filtered.filter(product => productMatchesCategorySlug(product, selectedCategory))
    }
      ...
      <div className="container-luxury py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="heading-luxury text-3xl lg:text-4xl mb-4">
            {selectedCategory ? (categories.find(c => c.value === selectedCategory)?.label ?? selectedCategory) : 'Nossa Coleção'}
          </h1>
          <p className="text-luxury text-lg">
            {selectedCategory 
              ? `Descubra a nossa seleção únicos e elegantes`
